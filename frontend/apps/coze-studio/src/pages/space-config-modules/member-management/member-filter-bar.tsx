...
import type { MemberFilterKey } from './types';
import { MEMBER_FILTERS } from './constants';

interface MemberFilterBarProps {
  memberRoleFilter: MemberFilterKey;
  onMemberRoleFilterChange: (value: MemberFilterKey) => void;
  ...
  <div className="flex flex-wrap items-center justify-between gap-[12px]">
    <div className="flex items-center">
      <Select
        ...
        optionList={MEMBER_FILTERS.map(filter => ({
          value: filter.key,
          label: I18n.t(filter.i18nKey, {}, filter.fallback),
        }))}
        onChange={value => onMemberRoleFilterChange(value as MemberFilterKey)}
      />
    </div>
    <div className="flex flex-wrap items-center justify-end gap-[10px]">
      <Input
        value={memberSearchKeyword}
        onChange={onMemberSearchKeywordChange}
        className="w-[220px] md:w-[300px]"
        showClear
        placeholder={I18n.t(
          'space_config_member_search_placeholder',
          {},
          '搜索昵称/用户名',
        )}
      />
