...
| 里程碑 | 时间 | 完成定义 | 当前状态 |
|---|---|---|---|
| M1 需求冻结 | Day 1-2 | PRD、原型、口径、任务拆解达成一致 | 进行中 |
| M2 后端可用 | Day 3-7 | 聚合/明细/预算/统计接口可联调 | 进行中 |
| M3 前端可用 | Day 6-10 | 两个 Tab 页面与交互可运行 | 进行中 |
| M4 联调测试 | Day 11-13 | 核心问题关闭，验收用例通过 | 未开始 |
...
| FE-04 | 前端 | 账单明细页（表格/排序/搜索/分页） | FE | BE-03 | 操作可用且结果正确 | P0 | 进行中 |
| FE-05 | 前端 | 导出交互（导出中/完成态） | FE | BE-04 | 导出反馈完整 | P0 | 进行中 |
| FE-06 | 前端 | 预算页（阈值配置+保存） | FE | BE-05 | 配置可保存并回显 | P0 | 进行中 |
| FE-07 | 前端 | 统计页（概览卡/趋势/排行） | FE | BE-06, BE-07 | 统计维度可切换 | P0 | 进行中 |
| FE-08 | 前端 | 加载态/空态/错误态/重试统一 | FE | FE-03, FE-04, FE-07 | 状态处理符合规范 | P0 | 进行中 |
...
2. `FE-04`：账单明细首版 UI 与请求链路已落地，helper 级与面板级测试已补齐，并补上并发回包保护与越页自动回退；当前待运行态联调与 `QA-02` 回归验证。
3. `FE-05`：导出交互已补齐轮询、立即完成态、过期态与下载拦截，hook 级与面板级自动化测试已覆盖；当前待浏览器联调真实下载文件内容、过期后端返回与 `QA-03` 回归验证。
4. `FE-06`：预算与阈值面板已补齐 helper / hook / panel 三层自动化回归，并新增乱序回包保护、保存中防并发与禁用态处理；当前待浏览器联调保存/回显链路，并与 `QA-04` 已完成的后端验收结果对齐前端表现。
5. `FE-07`：统计模块首版已落地，当前待浏览器联调概览卡/排行切换/分页链路，并确认趋势图所需接口补齐方案或 MVP 验收范围。
6. `FE-08`：统一状态层已接入主要面板，当前待浏览器联调空态/错误态/重置筛选动作，并评估是否覆盖更多导出/预算子态。

### 5.3 下一步（Top 6）
1. 执行 `QA-01` 运行态验收：验证 `AdminEmails` 配置/空配置两种场景下的平台入口显隐、页面访问与平台接口越权响应。
2. 收口 `FE-04`：补运行态联调，确认搜索/排序/分页参数与返回结果符合 `BE-03` 契约。
3. 收口 `FE-05`：浏览器内验证导出中、完成态下载、失败重试与过期提示。
4. 收口 `FE-06`：浏览器内验证预算规则查询、单行保存、启用开关与阈值回显。
5. 收口 `FE-07`：浏览器内验证统计概览与排行切换链路，并与 PM/BE 确认趋势图接口补齐或验收口径调整。
6. 收口 `FE-08`：浏览器内验证统一空态/错误态/重置筛选交互是否符合 PRD，并补权限空态的人工回归。
